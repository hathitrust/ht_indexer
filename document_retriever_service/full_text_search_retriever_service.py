...
def main():
    ...
    if len(list_documents) == 0:
        ...
        mysql_table = "ht_indexer_tracktable"
        if not init_args_obj.db_conn.table_exists(mysql_table):
            logger.error(f"{mysql_table} does not exist")
            init_args_obj.db_conn.create_table(ht_indexer_tracktable.HT_INDEXER_TRACKTABLE)
